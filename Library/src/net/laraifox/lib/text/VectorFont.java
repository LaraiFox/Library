...
public class VectorFont {
	...
	private int correctLeft = 9;
	private int correctRight = 8;
	...
	public VectorFont(Font font, boolean antiAlias, char[] additionalChars) {
		this.font = font;
	...
	public VectorFont(Font font, boolean antiAlias) {
	...
	private BufferedImage getFontImage(char ch) {
		...
		Graphics2D graphics = (Graphics2D) tempfontImage.getGraphics();
		if (antiAlias == true) {
	...
	private void createSet(char[] customCharsArray) {
		...
		try {
			...
			Graphics2D graphics = (Graphics2D) imgTemp.getGraphics();
	...
	public void drawString(float x, float y, String whatchars, int startIndex, int endIndex, float scaleX, float scaleY, int format) {
		...
		case ALIGN_CENTER: {
			for (int l = startIndex; l <= endIndex; l++) {
				...
				totalwidth += intObject.width - correctLeft;
			}
			totalwidth /= -2;
		}
		case ALIGN_LEFT:
		...
		while (i >= startIndex && i <= endIndex) {
			...
			if (intObject != null) {
				...
				if (charCurrent == '\n') {
					...
					drawQuad((totalwidth + intObject.width) * scaleX + x, startY * scaleY + y, totalwidth * scaleX + x, (startY + intObject.height) * scaleY + y, intObject.storedX + intObject.width, intObject.storedY + intObject.height,
